pipeline{
  ...
  stages{
    ...
    stage('copy war file to devenv and qa env') {
      ...
      steps {
        sh 'sudo scp -o StrictHostKeyChecking=no -r /mnt/wars/*.war kshiteej@${devip}:/mnt/wars'
         sh 'sudo scp -o StrictHostKeyChecking=no -r /mnt/wars/*.war kshiteej@${qaip}:/mnt/wars'
      }
    }
  }
}
